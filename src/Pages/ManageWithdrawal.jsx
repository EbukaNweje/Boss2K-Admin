...
const ManageWithdrawal = () => {
  ...
  const sendSignUpEmail = async (id) => {
    const urlemail = `https://boss2k.onrender.com/api/confirmwithdrawalemailsend/${id}`
    axios.post(urlemail)
       .then(response => {
    ...
    const acceptDeposit = (withdrawId) => {
        const url = `https://coinstarpro-bitminers-new-backnd-three.vercel.app/api/confirm-withdrawal/${withdrawId}`;
        axios.post(url)
            .then((response) => {
    ...
    const getallDeposit = () => {
        const url = "https://coinstarpro-bitminers-new-backnd-three.vercel.app/api/allwithdrawal";
        axios.get(url)
            .then((response) => {
